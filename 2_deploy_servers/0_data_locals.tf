...
locals {
  ...
  user_xpert_urls = { for k1, v1 in local.user_xpert_ids : 
    k1 => { for k2, v2 in v1 :
        k2 => "${v2}-api.${local.dns_domain}"
    }
  }
  ...
  user_xpert_ids        = data.terraform_remote_state.deploy_fortigates.outputs.user_xpert_ids
